...
local _pdStationPolys = {
	{
		points = {
			vector2(444.6608, -986.3198),
			vector2(444.6859, -994.2648),
			vector2(447.6199, -993.6270),
			vector2(448.0734, -987.2692),
		},
		options = {
			name = "pdstation_missionrow",
			minZ = 28.36417388916,
			maxZ = 32.414678573608,
		},
		data = { pdstation = true },
	},
	{
...
AddEventHandler("Core:Shared:Ready", function()
	exports['mythic-base']:RequestDependencies("Police", {
		...
		Interaction:RegisterMenu("police-utils", "Police Utilities", "tablet", function(data)
			Interaction:ShowMenu({
				...
				},
				{
					...
					action = function()
						Interaction:Hide()
						TriggerEvent("cm-k9:client:CommandsMenu")
					end,
					shouldShow = function()
						return LocalPlayer.state.onDuty == "police"
					end,
				},
				{
					...
					action = function()
						Interaction:Hide()
                        Notification:Custom("5 Minute Pit Timer", 60 * 1000 * 5, 'car', {
                            alert = {
                                background = "#247BA5B3",
                            },
                            progress = {
				...
				then
					if GetEntitySpeed(LocalPlayer.state.ped) > 2.0 then
						...
						if tarPlayer ~= 0 and dist <= 2.0 and GetGameTimer() - lastTackle > 7000 then
							...
							lib.requestAnimDict('swimming@first_person@diving', 0)
		...
		Targeting.Zones:AddBox("pd-clockinoff-mrpd", "clipboard-list", vector3(444.8730, -990.3134, 30.7074), 1.2, 1.2, {
			heading = 356,
			minZ = 28.49,
			maxZ = 32.49,
		...
		Targeting.Zones:AddBox("police-shitty-locker", "land-mine-on", vector3(467.1968, -1009.9062, 30.7074), 1.0, 3.8, {
			heading = 0,
			--debugPoly=true,
			minZ = 29.69,
			maxZ = 32.69,
...
RegisterNetEvent("Police:Client:Breached", function(type, id)
	...
	_breached[type][id] = GlobalState.OS_Time + (60 * 5)
end)
